...
function renderTopic3FusionQuestion(q) {
    ...
    const piecesHtml = meta.pieces.map((piece, idx) => {
        ...
        return `
            ...
            <div class="${anim} ${meta.level === 2 && idx === 1 ? 'min-w-[150px] md:min-w-[190px] px-7 md:px-9' : 'min-w-[82px] md:min-w-[104px] px-5'} py-3 md:py-4 rounded-2xl bg-gradient-to-br ${p[0]} ${p[1]} border-2 ${p[2]} ${p[3]} text-[1.69rem] md:text-[2.03rem] font-black shadow-sm text-center whitespace-nowrap">
                ${escapeHtml(piece)}
            </div>`;
    }).join('');
    ...
    const optionsHtml = q.options.map((opt, idx) => `
        <button
            ...
            class="option-btn min-h-[62px] px-4 py-2.5 bg-pink-50/40 hover:bg-pink-100/70 border-2 border-pink-200 rounded-2xl font-black text-gray-800 text-center transition-all text-[1.125rem] md:text-[1.35rem] shadow-xs pastel-btn">
            ${escapeHtml(opt)}
        </button>
    `).join('');
...
function loadQuestion() {
    ...
    if (questionBox) {
        questionBox.style.justifyContent = isTopic2Compact ? 'flex-start' : 'center';
        questionBox.style.paddingTop = isTopic2Compact ? '0.75rem' : '';
        questionBox.style.paddingBottom = isTopic2Compact ? '0.25rem' : '';
    }
