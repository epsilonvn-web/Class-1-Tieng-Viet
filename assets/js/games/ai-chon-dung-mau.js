...
function rcEnsureStyles(){
 ...
 s.textContent=`@keyframes rcShake{0%,100%{transform:translateX(0)}25%{transform:translateX(-7px)}50%{transform:translateX(7px)}75%{transform:translateX(-4px)}}.rc-shake{animation:rcShake .3s ease}#rc-picture{width:100%;height:245px;object-fit:contain}@media(max-width:900px){#rc-picture{height:215px}}@media(max-width:640px){#rc-picture{height:180px}}`;
...
function rcRender(){
 ...
 <div class="flex justify-between mb-2"><span class="px-3 py-1.5 rounded-full bg-white border border-pink-200 text-pink-600 font-black text-xs md:text-sm">🎨 Lượt ${rcRound}</span><div class="flex gap-2"><span class="px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-amber-700 font-black text-xs md:text-sm">⭐ ${rcScore}</span><span class="px-3 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-rose-600 font-black text-xs md:text-sm">🔥 x${rcStreak}</span><span class="px-3 py-1.5 rounded-full bg-purple-50 border border-purple-200 text-purple-600 font-black text-xs md:text-sm">🏆 ${rcBest}</span></div></div>
 <div class="text-center mb-2"><div class="text-base md:text-lg font-black text-teal-700">Đồ vật nào có <span class="text-fuchsia-500">màu đúng</span> như trong tranh?</div></div>
 <div class="relative bg-white rounded-3xl border-2 border-pink-200 overflow-hidden shadow-sm"><img id="rc-picture" src="${rcCurrent.image}" alt="${rcCurrent.name}"><div class="absolute left-3 bottom-3 px-3 py-1 rounded-full bg-white/95 border border-pink-200 text-pink-600 text-xs font-black">${rcCurrent.name}</div></div>
 <div class="text-center mt-2 mb-2 text-sm md:text-base font-black text-slate-700">Bé chọn <span class="text-fuchsia-600">đồ vật + màu sắc</span> khớp với tranh nhé!</div>
 <div class="grid grid-cols-2 gap-2.5">${rcQuestion.options.map((o,i)=>`<button onclick="rcChoose(this,${o.correct})" class="min-h-[58px] px-4 py-2 rounded-2xl border-2 ${p[i]} text-slate-700 font-black text-sm md:text-base shadow-sm">${o.text}</button>`).join('')}</div>
